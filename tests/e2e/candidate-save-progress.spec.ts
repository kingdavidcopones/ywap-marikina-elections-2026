...
test('candidate save buttons stay busy until the server confirms add and edit', async ({page}) => {
  ...
  const voters = [{memberId: 'YWAP-1', name: 'Test Voter', firstName: 'Test', lastName: 'Voter',
    ageGroup: 'Young Adults', eligible: true, attributes: {member_id: 'YWAP-1'}}];
  ...
  await expect(page.getByText('Test Voter', {exact: true}).first()).toBeVisible();

  await page.getByRole('button', {name: 'Edit Test Voter'}).click();
