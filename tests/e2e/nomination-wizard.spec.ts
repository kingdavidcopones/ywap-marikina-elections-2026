...
test('nomination wizard reviews choices, confirms submission, and keeps one card width', async ({page}) => {
  let submitted: {ageGroup: string; name: string; choices: Record<string, {name: string; youthRecordId?: string}>} | null = null;
  ...
  expect(submitted).toEqual({ageGroup: 'teens', name: '', choices: {
