-- A custom CSV filter is the complete eligibility rule for its position.
-- Positions without a custom filter retain their age-group scope.
...
returns uuid[] language sql stable security invoker set search_path = '' as $$
  ...
  where p.election_id = p_election_id
    ...
    and (p.voting_rule->>'type' = 'custom' or p.group_scope = 'general' or p.group_scope = v.age_group::text);
$$;

-- Use the same eligibility function for both ballot display and submission.
-- This keeps validation and persisted selections in sync with the voter view.
