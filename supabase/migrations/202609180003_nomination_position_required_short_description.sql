-- Existing positions default to optional (required = false); an admin opts a
-- position into blocking submission via the "Required field in nomination"
-- checkbox. The old role-details toggle and its two long-form fields are
-- replaced by a single short description shown directly on the public form.
alter table public.nomination_positions
  add column required boolean not null default false,
  add column short_description text not null default '',
  add constraint nomination_position_short_description_length check (
    char_length(short_description) <= 80
  );

alter table public.nomination_positions
  drop column show_role_details,
  drop column about_role,
