...
export async function GET(_request: Request, context: {params: Promise<{id: string}>}) {
  ...
  try {
    ...
    const csv = buildCsv(['Name', 'Nominee', 'Position nominated', 'Nominated by', 'Submitted at'],
      nominees.map((entry) => [entry.nomineeName, entry.nomineeEligible ? 'YES' : '', entry.positionName, entry.nominatorName || '', entry.submittedAt]));
